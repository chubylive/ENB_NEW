    ...
    switch (packet[0]) {
        ...
        case HCI_EVENT_COMMAND_COMPLETE:
            ...
            if (COMMAND_COMPLETE_EVENT(packet, hci_read_bd_addr)) {
                bd_addr_t addr1;
                bt_flip_addr(addr1, &packet[OFFSET_OF_DATA_IN_COMMAND_COMPLETE + 1]);
                log_info("Local Address, Status: 0x%02x: Addr: %s\n",
                    packet[OFFSET_OF_DATA_IN_COMMAND_COMPLETE], bd_addr_to_str(addr1));
            }
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE:
            if (!packet[2]){
                ...
                hci_connection_t * conn1 = connection_for_handle(handle);
                if (conn1) {
                    hci_shutdown_connection(conn1);
                }
            }
            break;
...
int hci_send_device_int(void){
	uint8_t tx_payload[] = {0x00,0xFE,0x26,0x09,0x04,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01,0x00,0x00,0x00};
...
int gap_send_cmd(const gap_cmd_t *cmd, ...){
	va_list argptr;
	va_start(argptr, cmd);
	uint16_t size = hci_create_cmd_internal(hci_stack.hci_packet_buffer, (const hci_cmd_t *)cmd, argptr);
	va_end(argptr);
	return hci_send_cmd_packet(hci_stack.hci_packet_buffer, size);
